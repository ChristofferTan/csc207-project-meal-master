...
public class GroceryListInteractorTest {
    ...
    public void SuccessTest() {
        ...
        Planner planner = plannerfactory.create("Faraaz");
        plannerDAO.saveNewPlanner(planner);

        User user1 = userFactory.create("Faraaz", "123", "faraaz", 18, "Man", 170, 68, plannerDAO.getPlanner("Faraaz"));
        userDAO.save(user1);
        ...
        plannerDAO.save("Faraaz", DayOfWeek.MONDAY, MealType.BREAKFAST, recipe);

        GroceryListInputData inputData = new GroceryListInputData("Faraaz");

        GroceryListOutputBoundary successPresenter = new GroceryListOutputBoundary() {
            @Override
            public void prepareSuccessView(GroceryListOutputData groceryListOutputData) {
                assertTrue(groceryListOutputData.getGroceryList().contains("2 tablespoons olive oil"));
                assertTrue(groceryListOutputData.getGroceryList().contains("4 bone-in"));
                assertTrue(groceryListOutputData.getGroceryList().contains("skin-on chicken thighs (about 1 pound)"));
                assertTrue(groceryListOutputData.getGroceryList().contains("Salt and pepper"));
                assertTrue(groceryListOutputData.getGroceryList().contains("4 scallions"));
                assertTrue(groceryListOutputData.getGroceryList().contains("white and green parts separated and thinly sliced"));
                assertTrue(groceryListOutputData.getGroceryList().contains("2 tablespoons finely chopped peeled fresh ginger"));
                assertTrue(groceryListOutputData.getGroceryList().contains("3 garlic cloves"));
                assertTrue(groceryListOutputData.getGroceryList().contains("minced"));
                assertTrue(groceryListOutputData.getGroceryList().contains("2 ounces shiitake mushroom caps (about 3/4 cups)"));
                assertTrue(groceryListOutputData.getGroceryList().contains("thinly sliced"));
                assertTrue(groceryListOutputData.getGroceryList().contains("1 1/2 cups Arborio rice"));
                assertTrue(groceryListOutputData.getGroceryList().contains("3 2/3 cups chicken broth"));
                assertTrue(groceryListOutputData.getGroceryList().contains("Fresh cilantro for serving"));
            }

        };

        GroceryListInputBoundary interactor = new GroceryListInteractor(plannerDAO, successPresenter);
